...
(function($) {
    ...
    $(function() {    		
	    ...
	    // get primary menu title and add to link element if exists	 
	    $('#primary-menu li a').each(function() {	    	
			var menuText = $(this).text();
			var menuTitle = $(this).attr('title');
			if (menuTitle !== undefined) {	    			
				$(this).html(menuText + '<br /><span>' + menuTitle + '</span>').addClass('custom-title');
			} 										
		});
		// add margin to primary menu if blog name is long
		var blogNameWidth = $('#blog-name').width();
		if (blogNameWidth > 278) {
			$('#primary-menu').css('float','right');
		}		
		// add glyphicon to widget titles
		$('.widget-title').each(function() {
	    	var menuTitle = $(this).text();					
			$(this).html('<span class="glyphicon glyphicon-circle-arrow-right"></span>' + menuTitle);
		});	
		// add glyphicon to image captions
		$('.wp-caption-text').each(function() {
			var captionText = $(this).text();
			$(this).html('<span class="glyphicon glyphicon-info-sign"></span>' + captionText);			
		});
