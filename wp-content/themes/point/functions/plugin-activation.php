    ...
    class TGMPA_List_Table extends WP_List_Table {
        ...
        protected function _gather_plugin_data() {
            ...
            foreach ( TGM_Plugin_Activation::$instance->plugins as $plugin ) {
                ...
                $table_data[$i]['available_separately'] = false;
                if ( ! empty( $plugin['source'] ) && preg_match( '|^/|', $plugin['source'] ) && !file_exists( $plugin['source'] ) ) {
                    $table_data[$i]['available_separately'] = true; // Possibly pre-packaged, but not available
                    $plugin['available_separately'] = true;
                    if ( ! empty( $plugin['info_link'] ) )
                        $table_data[$i]['info_link'] = $plugin['info_link'];
                }
                ...
                elseif ( ! $source || preg_match( '|^http://wordpress.org/extend/plugins/|', $source ) ) {
                    ...
                    if ( !empty( $plugin['info_link'] )) {
                        $table_data[$i]['plugin'] = '<strong><a href="' . esc_url( $plugin['info_link'] ) . '" title="' . esc_attr( $plugin['name'] ) . '" target="_blank">' . $plugin['name'] . '</a></strong>';
                    } else {
                ...
                if ( !empty ( $plugin['available_separately'] ) ) {
                    $table_data[$i]['source'] = __( 'Available Separately', 'mythemeshop' );
                }
        ...
        public function column_plugin( $item ) {
            ...
            if ( ! isset( $installed_plugins[$item['file_path']] ) && empty( $item['available_separately'] ) ) {
            ...
            elseif ( isset( $installed_plugins[$item['file_path']] ) && is_plugin_inactive( $item['file_path'] ) ) {
                $actions = array(
                    'activate' => sprintf(
            ...
            // We need to display the 'Get it' hover link.
            elseif ( $item['available_separately'] ) {
                $actions = array(
                    'get_it' => sprintf(
                        '<a href="%1$s" title="' . esc_attr__( 'Get ', 'mythemeshop' ) . ' %2$s" target="_blank">' . __( 'Get it', 'mythemeshop' ) . '</a>',
                        esc_url($item['info_link']),
                        $item['sanitized_plugin']
                    ),
                );
            }
