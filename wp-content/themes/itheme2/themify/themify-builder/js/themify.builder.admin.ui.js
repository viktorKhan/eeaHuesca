	...
	var ThemifyPageBuilder = {
		...
		setColorPicker: function(context) {
			...
			$('body').on('blur', '.colordisplay', function(){
				...
				$field.val( $field.val().replace(/[abcdef0123456789]{3,6}/i, tempColor.replace('#', '')) );
		...
		moduleOptAddRow: function(e) {
			...
			template.find('.builderColorSelectInput').minicolors('destroy')
			.wrap( '<div class="themify_builder_input" />' )
			.before( '<span class="builderColorSelect"><span></span></span>' );
			self.setColorPicker(template);
